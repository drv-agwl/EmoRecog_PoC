...
class TransformerDecoder(nn.Module):
    """
    Transformer decoder consisting of *args.encoder_layers* layers. Each layer
    ...
    def __init__(self, embed_dim, num_heads, layers, attn_dropout=0.0, relu_dropout=0.0, res_dropout=0.0,
                 embed_dropout=0.0, attn_mask=False, batch_size=2):
        super().__init__()
        self.dropout = embed_dropout  # Embedding dropout
        self.attn_dropout = attn_dropout
        self.embed_dim = embed_dim
        ...
        self.layers = nn.ModuleList([])
        for layer in range(layers):
            new_layer = TransformerDecoderLayer(embed_dim,
                                                num_heads=num_heads,
                                                attn_dropout=attn_dropout,
                                                relu_dropout=relu_dropout,
                                                res_dropout=res_dropout,
...
class TransformerDecoderLayer(nn.Module):
    ...
    *args.encoder_normalize_before* to ``True``.
    ...
    def forward(self, tgt, x_k, x_v, query_embed):
        ...
        residual = tgt
        tgt = self.maybe_layer_norm(1, tgt, before=True)
        tgt = F.relu(self.fc1(tgt))
        tgt = F.dropout(tgt, p=self.relu_dropout, training=self.training)
        tgt = self.fc2(tgt)
        tgt = F.dropout(tgt, p=self.res_dropout, training=self.training)
        tgt = residual + tgt
        tgt = self.maybe_layer_norm(1, tgt, after=True)
        return tgt
