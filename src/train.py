...
def train_model(settings, hyp_params, train_loader, valid_loader, test_loader):
    ...
    def train(model, optimizer, criterion, ctc_a2l_module, ctc_v2l_module, ctc_a2l_optimizer, ctc_v2l_optimizer,
        ...
        for i_batch, (batch_X, batch_Y, batch_META) in enumerate(train_loader):
            ...
            if batch_chunk > 1:
                ...
                if hyp_params.dataset == 'iemocap':
                    if hyp_params.classifier == "MLP":
                        preds = preds.view(-1, 2)
                        eval_attr = eval_attr.view(-1)
                    elif hyp_params.classifier == "Transformer":
            ...
            for i_batch, (batch_X, batch_Y, batch_META) in enumerate(loader):
                ...
                if hyp_params.dataset == 'iemocap':
                    if hyp_params.classifier == "MLP":
                        ...
                        temp = eval_attr
                        eval_attr = torch.argmax(eval_attr, axis=1)
                total_loss += criterion(preds, eval_attr).item() * batch_size

                # Collect the results into dictionary
                if hyp_params.classifier == "Transformer":
                    emotion_class = torch.argmax(preds, dim=-1)
                    preds = torch.zeros((preds.shape[0], preds.shape[1], 2))
                    for batch_idx in range(preds.shape[0]):
                        for emotion_idx in range(preds.shape[1]):
                            if emotion_idx == emotion_class[batch_idx]:
                                preds[batch_idx][emotion_idx][0] = 0.
                                preds[batch_idx][emotion_idx][1] = 1.
                            else:
                                preds[batch_idx][emotion_idx][0] = 1.
                                preds[batch_idx][emotion_idx][1] = 0.
                    eval_attr = temp
    ...
    epoch_nos = [i for i in range(1, len(train_losses) + 1)]
    plt.plot(epoch_nos, train_losses, label="Train")
    plt.plot(epoch_nos, val_losses, label="Val")
